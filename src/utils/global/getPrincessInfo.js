...
class Princess {
  ...
  _getPrincessStatus() {
    this.status = this.db.query("select unit_name,rarity,atk_type,search_area_width,normal_atk_cast_time,comment from unit_data where unit_id=?", this.id)[0];
  }

  _getPrincessProfile() {
    this.profile = this.db.query("select age,guild,race,height,weight,birth_month,birth_day,blood_type,favorite,voice,catch_copy,self_text,guild_id from unit_profile where unit_id=?", this.id)[0];
    this.profile.fullname = this.db.query("select unit_name from unit_background where unit_id=?", this.id)[0].unit_name;
  }
  ...
  _getPrincessGrowth(){
    ...
    this.db.query("select * from unit_rarity where unit_id=?", this.id).map(x=>{
      delete x.unit_id;
      let _rarity = x.rarity;
      delete x.rarity;
      this.growth.rarity[_rarity] = x;
    });
    this.db.query("select * from unit_promotion_status where unit_id=?", this.id).map(x=>{
      delete x.unit_id;
  ...
  _getPrincessAttackPattern() {
    this.attack_pattern = this.db.query("select * from unit_attack_pattern where unit_id=?", this.id)[0];
  }

  _getPrincessPromotion() {
    this.promotion_info = new Object();
    let row = this.db.query("select * from unit_promotion where unit_id=?", this.id);
    row.forEach((level) => {
  ...
  // _getPrincessSkillAction(action_id) {
  //   return action_id === 0 ? 0 : this.db.query("select * from skill_action where action_id=?", action_id)[0];
  // }

  _getPrincessSkillData() {
    this.skill = new Object();
    let skill_info = this.db.query("select * from unit_skill_data where unit_id=?", this.id)[0];
    this.skill = {
  ...
  _getPrincessSkillDetail(skill_id) {
    ...
    let row = this.db.query("select * from skill_data where skill_id=?", skill_id)[0];
    const getSkillAction = (action_id) => {
      return action_id === 0 ? 0 : this.db.query("select * from skill_action where action_id=?", action_id)[0];
    }
    detail = {
      name: row.name,
      ...
      actions: {
        action_list: Array(row.action_1, row.action_2, row.action_3, row.action_4, row.action_5, row.action_6, row.action_7).map(getSkillAction),
        depend_action_list: Array(row.depend_action_1, row.depend_action_2, row.depend_action_3, row.depend_action_4, row.depend_action_5, row.depend_action_6, row.depend_action_7).map(getSkillAction)
      },
      icon: row.icon_type
    }
    return detail;
  ...
  _getPrincessUniqueEquipment() {
    ...
    let uniq_equip_data = this.db.query("select * from unique_equipment_data where equipment_id=?", equip_id);
    let uniq_equip_enh_rate = this.db.query("select * from unique_equipment_enhance_rate where equipment_id=?", equip_id);
  ...
  list() {
    let list = new Array();
    for (let item of this.db.query("select unit_id from unit_data where unit_id like '10%'").values()){
